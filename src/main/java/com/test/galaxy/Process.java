...
public class Process
{
    ...
    public void parseInput() throws Exception
    {
    	...
    	try(BufferedReader br = new BufferedReader(new FileReader("Input.txt")))
    	{
    		...
    		{
    			// Get the User Input
    			if (StringUtils.isNotNullOrEmpty(currentLine) && currentLine.startsWith(Constants.HOW))
    			{
    ...
    {
        if ((theInput.startsWith(Constants.HOW_MUCH) || theInput.startsWith(Constants.HOW_MANY)))
        {
    ...
    private void parseAssertion(String theInput)
    {
        ...
        {
            String[] aInputSplits = theInput.split(Constants.IS_VALUE);
            
            if (aInputSplits.length > 1 && StringUtils.isNotNullOrEmpty(aInputSplits[1]))
            {
                if (aInputSplits[1].endsWith(Constants.CREDITS))
                {
                    double aFinalCredits = 0;
                    String[] split = aInputSplits[0].split(Constants.SPACE);
                    ...
                    for (String aGalacticUnit : split)
                    {
                        ...
                        RomanNumber romanNumerial = itsDictionary.getRomanNumerialValue(value);
                    ...
                    String[] aCreditSplit = aInputSplits[1].split(Constants.SPACE);
