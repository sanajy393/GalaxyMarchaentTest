...
public class RomanConversion 
{
	...
	public double convertRomanNumerialToValue(List<Object> aRomanValueList)
	{
		...
		if (aRomanValueList != null && aRomanValueList.size() >= 1) 
		{
			if (aRomanValueList.get(0) instanceof String) 
			{
				...
				RomanNumber romanNumerial = getRomanNumerialValue(aFirstValue);
		...
		for (int i = 1; i < aRomanValueList.size() ; i++) 
		{
			...
			if (aFirstObjectValue instanceof String) 
			{
				...
				RomanNumber aSecondNumerial = getRomanNumerialValue(aFirstValue);
				...
				else 
				{
					...
					if(i < aRomanValueList.size())
					{
						if(aRomanValueList.get(i) instanceof String)
						{
							...
							RomanNumber aNewTempNumerial = getRomanNumerialValue(aNewTempElement);
	...
	{
		if (RomanNumber.V.name().equalsIgnoreCase(toValue)
				|| RomanNumber.L.name().equalsIgnoreCase(toValue)
				|| RomanNumber.D.name().equalsIgnoreCase(toValue)) 
		{
		...
		case "I":
			if (fromValue.equalsIgnoreCase(RomanNumber.V.name())
					|| fromValue.equalsIgnoreCase(RomanNumber.X.name())) 
			{
				return true;
			} 
			else
		...
		case "X":
			if (fromValue.equalsIgnoreCase(RomanNumber.L.name())
					|| fromValue.equalsIgnoreCase(RomanNumber.C.name())) 
			{
				return true;
			} 
			else 
		...
		case "C":
			if (fromValue.equalsIgnoreCase(RomanNumber.D.name())
					|| fromValue.equalsIgnoreCase(RomanNumber.M.name())) 
			{
				return true;
			} 
			else 
